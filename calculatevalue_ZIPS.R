...
#list of file names
filelocs <- sapply("./zips/", paste, list.files("./steamer"), sep="")[c(1:6,8)]
...
#read in projections
pitcher_projections <- read.csv("./zips/pitchers.csv", stringsAsFactors=FALSE)

steamer <- read.csv("./steamer/pitchers.csv", stringsAsFactors=FALSE) %>%
      select(Name, SV)

#keep only relevant columns
pitcher_projections <- select(pitcher_projections,Name,Team,W,ERA,IP,SO,WHIP,playerid) %>%
      mutate(position = "pitcher") %>%
      left_join(steamer) %>%
      select(Name,Team,W,ERA,SV,IP,SO,WHIP,playerid, position)      
...
save(hitter_projections, pitcher_projections, file = "projections_ZIPS.rda")
